...
import { fetchMyBookings, fetchBookingQR, getTicketPdfUrl, cancelBooking } from './api'
...
export default function BookingHistory() {
  ...
  const load = () => { setLoading(true); fetchMyBookings({ page, limit: 10 }).then(setData).catch(console.error).finally(() => setLoading(false)) }
  useEffect(() => { load() }, [page])

  const handleCancel = async ref => { if (confirm('Cancel this booking?')) { await cancelBooking(ref); load() } }

  return (
    <div className="page">
      <h2>My Bookings</h2>
      {loading ? <p className="loading">Loading your bookings...</p> : data.items.length === 0 ? (
        <div className="empty"><p style={{ fontSize: '3rem', marginBottom: 12 }}>🎟️</p><p>You haven't booked any tickets yet.</p><Link to="/" className="btn btn-primary" style={{ marginTop: 16 }}>Browse Movies</Link></div>
      ) : (
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {data.items.map(b => <TicketCard key={b.booking_reference} booking={b} onCancel={handleCancel} />)}
          </div>
          {data.total_pages > 1 && (
...
function TicketCard({ booking, onCancel }) {
  ...
  useEffect(() => { fetchBookingQR(booking.booking_reference).then(r => setQr(r.qr_base64)).catch(() => {}) }, [booking.booking_reference])
  ...
  return (
    <div className="ticket-card">
      <div className="ticket-left">
        ...
        <div className="ticket-seats">{booking.seats.map(s => <span key={s} className="ticket-seat-badge">{s}</span>)}</div>
      </div>
      ...
      <div className="ticket-right">
        ...
        <div className="ticket-actions">
          <button className="btn btn-outline btn-sm" onClick={() => setModal(true)}>View</button>
          <a href={pdfUrl} className="btn btn-primary btn-sm" download>PDF</a>
          {booking.status === 'confirmed' && <button className="btn btn-danger btn-sm" onClick={() => onCancel(booking.booking_reference)}>Cancel</button>}
        </div>
      </div>
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="preview-ticket">
              <div className="preview-header"><span className="preview-logo">🎬 MovieBooker</span><span className="preview-label">CINEMA TICKET</span></div>
              ...
              <div className="preview-details">
                ...
                <div className="preview-row"><span>Ref</span><span className="ref-highlight">{booking.booking_reference}</span></div>
                <div className="preview-row"><span>Booked</span><span>{new Date(booking.booking_time).toLocaleString('en-IN')}</span></div>
              </div>
            ...
            <div className="modal-actions"><a href={pdfUrl} className="btn btn-primary" download>📥 Download PDF</a></div>
          </div>
        </div>
      )}
    </div>
